...
+ master137：192.168.182.137
+ node138：192.168.182.138
+ node139：192.168.182.139
...
分发二进制文件到集群所有节点，创建文件etcd_dispatcher.sh：
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp etcd-v3.4.9-linux-amd64/etcd* root@${node_ip}:/opt/k8s/bin
...
分发生成的证书和私钥到各 etcd 节点，创建`etcd_cert_dispatcher.sh`：
...
创建`etcd_systemd_template.sh`并执行：
``` bash
cd /opt/k8s/work
source /opt/k8s/bin/environment.sh
...
创建`etcd_systemd_dispatcher.sh`并执行：
``` bash
cd /opt/k8s/work
source /opt/k8s/bin/environment.sh
for node_ip in ${NODE_IPS[@]}
...
部署完 etcd 集群后，创建`etcd_health.sh`并执行：
``` bash
cd /opt/k8s/work
source /opt/k8s/bin/environment.sh
for node_ip in ${NODE_IPS[@]}
...
``` bash
[root@master137 work]# bash etcd_health.sh 
>>> 192.168.182.137
https://192.168.182.137:2379 is healthy: successfully committed proposal: took = 68.91606ms
>>> 192.168.182.138
https://192.168.182.138:2379 is healthy: successfully committed proposal: took = 35.695135ms
>>> 192.168.182.139
https://192.168.182.139:2379 is healthy: successfully committed proposal: took = 41.558642ms
```
...
输出：
```
[root@master137 work]# /opt/k8s/bin/etcdctl \
>   -w table --cacert=/etc/kubernetes/cert/ca.pem \
>   --cert=/etc/etcd/cert/etcd.pem \
>   --key=/etc/etcd/cert/etcd-key.pem \
>   --endpoints=${ETCD_ENDPOINTS} endpoint status 
...
+------------------------------+------------------+---------+---------+-----------+------------+-----------+------------+--------------------+--------+
| https://192.168.182.137:2379 | dcaf38950000c9c1 |   3.4.9 |   20 kB |     false |      false |         2 |          8 |                  8 |        |
| https://192.168.182.138:2379 | 1e40f30792669e4c |   3.4.9 |   20 kB |      true |      false |         2 |          8 |                  8 |        |
| https://192.168.182.139:2379 | 7fc52a5ba4ec9cff |   3.4.9 |   20 kB |     false |      false |         2 |          8 |                  8 |        |
+------------------------------+------------------+---------+---------+-----------+------------+-----------+------------+--------------------+--------+

```
+ 可见，当前的 leader 为 192.168.182.138:2379 