...
``` bash
hostnamectl set-hostname master137 # 将 master137 替换为当前主机名
```
同理，在`138`和`139`上设置hostname

在每台机器的 `/etc/hosts` 文件中添加主机名和 IP 的对应关系：

``` bash
cat >> /etc/hosts <<EOF
192.168.182.137 master137
192.168.182.138 node138
192.168.182.139 node139
EOF
```
...
本操作只需要在 master137 节点上进行，设置 root 账户可以无密码登录**所有节点**：
``` bash
ssh-keygen -t rsa 
ssh-copy-id root@master137
ssh-copy-id root@node138
ssh-copy-id root@node139
```
...
```
原因为，k8s各组件需要通信，内网环境可以直接关闭，更好的做法是在防火墙上打卡各组件端口。
...
```
原因为，k8s scheduler会根据内存来调度容器，swap增加了此工作的成本，同时影响性能。
...
**以后所有的脚本都在/opt/k8s/work目录**
``` bash
#在/opt/k8s/work编写sh文件 env_dispatcher.sh
