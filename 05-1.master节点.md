...
注意：如果没有特殊指明，本文档的所有操作**均在master137 节点上执行**。
...
``` bash
cd /opt/k8s/work
wget https://dl.k8s.io/v1.18.0/kubernetes-server-linux-amd64.tar.gz  # 自行解决翻墙问题
tar -xzvf kubernetes-server-linux-amd64.tar.gz
cd kubernetes
tar -xzvf  kubernetes-src.tar.gz
```

创建`k8s_dispatcher.sh`将二进制文件拷贝到所有 master 节点：
